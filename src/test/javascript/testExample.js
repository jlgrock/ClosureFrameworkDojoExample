...
goog.require("com.github.ClosureFrameworkDojoExample");

/**
 * Note that all of your tests must start with the word "test"
 */
testHexDigitsExists = function() {
	assertTrue(goog.isDefAndNotNull(com.github.ClosureFrameworkDojoExample.hexDigits));
	assertEquals(16, com.github.ClosureFrameworkDojoExample.hexDigits.length);
};

testRotateColorFunction = function() {
	newParagraph = document.createElement('p');
	assertEquals(true, true);//TODO assert that this is the color black
	com.github.ClosureFrameworkDojoExample.rotateColorFunction
	assertEquals(true, true);//TODO assert that it has changed to #ffebcd
};

testConvertHex = function() {
	assertEquals("#ffebcd", com.github.ClosureFrameworkDojoExample.rgb2hex("rgb(255, 235, 205)"));
}
